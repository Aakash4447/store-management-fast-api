...
Revision ID: ddbb872beab8
Revises: 
Create Date: 2026-09-01 19:55:08.105166
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_store_owners_email'), 'store_owners', ['email'], unique=True)
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.ForeignKeyConstraint(['owner_id'], ['store_owners.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.Column('customer_address', sa.String(length=1000), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_store_owners_email'), table_name='store_owners')
